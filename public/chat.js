...
socket.on("created", function () {
  ...
  navigator.mediaDevices // enable media services.
    ...
    .then(function (mediaStream) {
      ...
      userVideo.srcObject = mediaStream; // initialized media information in userVideo video tag.
      userVideo.onloadedmetadata = function (e) {
        userVideo.play(); // once tag is ready we play it
      };
    })
    .catch(function (err) {
...
function OnTrackFunction(event) {
  peerVideo.srcObject = event.streams[0]; // initialized media information in peerVideo video tag. event.streams is an array has both video and audio streams ai 0,1 position respectively.
  peerVideo.onloadedmetadata = function (e) {
    peerVideo.play(); // once tag is ready we play it
  };
}
